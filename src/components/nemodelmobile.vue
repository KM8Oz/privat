  ...
  <div>
    ...
    >
      <q-step
        ...
        :done="step > 1"
      >
        <div class="q-gutter-y-md column w-80 text-h3">
          <q-input
            clearable
            maxlength="30"
            color="pink-6"
            ...
            :rules="[
              ...
              (val) =>
                [
                  ...
                  'yandex',
                  'mail',
                ].indexOf(val.split('@')[1].split('.')[0]) != -1 ||
                'email is invalid (inbox,gmail,yahoo,outlook,live,hotmail,yandex,mail)',
            ]"
          ...
          <q-input
            v-model="stepone.BthDay"
            mask="####/##/##"
            fill-mask
            :rules="[
              (val) => !!val || 'ПОЛЕ ОБЯЗАТЕЛЬНО',
              (val) =>
                Number(val.split('/')[1]) <= 12 ||
                '..../' + Number(val.split('/')[1]) + '/..',
              (val) =>
                val.split('/')[2] <= 31 ||
                '..../../' + Number(val.split('/')[2]),
              (val) =>
                val.split('/')[0] <= Number(resctDate.split('/')[0]) ||
                'ВОЗРАСТ ОГРАНИЧЕН (+18)',
            ]"
          >
     ...
     class="policy"
    >
              <span class="text-body3" v-html="policy"></span>
            </q-scroll-area>
              <q-toggle
                v-model="steptree.agree"
                label="согласен"
                keep-color
                :color="steptree.agree ? 'green' : 'pink-6'"
              />
            </div>
          </template>
        </q-field>
      ...
      <template v-slot:navigation>
        <q-stepper-navigation>
          <q-btn
            ...
            rounded
            no-caps
            class=" mt-vh-4 text-body1 q-px-md"
            :disabled="errors"
            color="pink-6"
          ...
          <q-btn
            ...
            rounded
            no-caps
            class=" mt-vh-4  text-body1 q-px-md q-mx-xs"
            :disabled="!steptree.agree"
            color="pink-6"
            label="Заключать"
            @click="finishup()"
          >
          ...
          <q-btn
            rounded
            no-caps
            class=" mt-vh-4 end text-body1 q-px-md q-mx-xs"
            v-if="step > 1"
            color="pink-6"
    ...
    <div class="backbtn"><q-btn class="w-100 q-my-md" @click="goback" icon="undo" color="#fddeff" text-color="black" /></div>
  </div>
</template>
 ...
 ...mapState(['pcm'])
    ...
    checkPassport: function (res) {
      if (
        [
          ...
          !!res.data.documentCode,
          !!res.data.documentNumberCheckDigit,
          !!res.data.birthDate,
          !!res.data.birthDateCheckDigit,
        ].indexOf(false) === -1
      ) {
        this.$q.notify({
          type: "positive",
  ...
  data() {
    return {
      ...
      resctDate: date.formatDate(
        date.subtractFromDate(new Date(), { year: 18 }),
        "YYYY/MM"
      ),
      step: 1,
      errors: true,
  ...
  methods: {
    ...
    finishup() {
      this.$socket.emit("newmodeldata", {
        token: pcm.user.tk,
        data: {
          passport: this.steptwo.data,
          form: this.stepone,
          agreement: this.steptree,
        },
      });
    },
    sData(res) {
      if (!this.errors) {
        this.$socket.emit("newmodeldata", {
          token: pcm.user.tk,
          data: { ...this.stepone, ...this.steptwo, ...this.steptree },
        });
      } else {
        this.$q.notify({
    ...
    sendImg() {
      this.steptwo.imgsrc.generateBlob((blob) => {
        this.$socket.emit("checkPassport", {
          token: pcm.user.tk,
          blobs: blob,
        });
      });
    ...
    onchange() {
      if (this.startverf) {
        this.errors =
          [
            document.getElementsByClassName("q-field--error").length > 0,
            this.stepone.email === "",
            this.stepone.name === "",
